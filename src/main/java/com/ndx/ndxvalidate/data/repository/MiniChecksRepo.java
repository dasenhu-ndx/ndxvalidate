...
public interface MiniChecksRepo extends JpaRepository<AccountRequest, Long> {
    ...
    @Query(nativeQuery = true, value = "EXEC  [dbo].[SP_NDXValidateCheckExistingCustomer]" +
            "@LabName = ?1" +
            ",@Address1 = ?2" +
    ...
    @Query(nativeQuery = true, value = "EXEC [dbo].[SP_NDXValidateCheckExistingDoctor]" +
            "@LabName = ?1" +
            ",@ExistingDoctorNumber = ?2" +
            ",@LastName = ?3" +
            ",@LicenseNumber = ?4" +
            ",@NPINumber = ?5")
    List<CheckExistingDoctor> getExistingDoctor(String labName, String docId, String lName, String npi);

    @Query(nativeQuery = true, value = "EXEC  [dbo].[SP_NDXValidateCheckSimilarCustomer] " +
            "@LabName = ?1" +
            ",@Address1 = ?2" +
            ",@ZipCode = ?3" +
            ",@OfficePhone = ?4")
    ...
    @Query(nativeQuery = true, value = "EXEC [dbo].[SP_NDXValidateCheckSimilarDoctor]" +
            "@LabName = ?1" +
            ",@FirstName = ?2" +
            ",@LastName = ?3" +
            ",@LicenseNumber = ?4" +
